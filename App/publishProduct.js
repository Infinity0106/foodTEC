...
export default class publishProduct extends Component {
  ...
  createProduct(json) {
    ...
    firebase
      ...
      .then(s => {
        ...
        if (
          this.state.photo &&
          this.state.photo.includes("data:image/jpeg;base64,")
        ) {
          ...
          window.Blob = Blob;
          window.Blob
            .build(this.state.photo.replace("data:image/jpeg;base64,", ""), {
          ...
          window.Blob = Blob;
          window.Blob
            .build(RNFetchBlob.wrap(this.state.photo), { type: "image/jpeg" })
            .then(blob => {
              var uploadTask = firebase
                .storage()
                .ref()
    ...
    if (this.state.photo == undefined) {
      return (
        <View style={styles.pt30}>
          <PlaceP width={150} height={150} />
        </View>
      );
    } else {
      return (
        <Image source={{ uri: this.state.photo }} style={styles.imageRender} />
      );
    }
  }
  ...
  publishInfo() {
    ...
    if (
      ...
      if (
        this.state.lugar == "Aulas" ||
        this.state.lugar == "Biblioteca" ||
        this.state.lugar == "Ciap" ||
        this.state.lugar == "Learning Commons"
      ) {
  ...
  render() {
    return (
      <View style={styles.container}>
        <Header />
        <View style={styles.container}>
          <KeyboardAwareScrollView extraHeight={40} extraScrollHeight={40}>
            ...
            <Grid style={styles.margins}>
              ...
              <Row size={10}>
                <Col>
                  <Text>Lugar: </Text>
                  <ModalDropdown
                    style={[styles.btnModal, { marginRight: 5 }]}
                    textStyle={styles.textbtnModal}
                    options={this.state.options1}
                    onSelect={this.changeOpt1.bind(this)}
                  />
                </Col>
                <Col>
                  {this.state.render2 && <Text>Posicion: </Text>}
                  {this.state.render2 && (
                    <ModalDropdown
                      defaultValue={"detalles"}
                      style={[styles.btnModal, { marginLeft: 5 }]}
                      textStyle={styles.textbtnModal}
                      options={this.state.options2}
                      onSelect={this.changeOpt2.bind(this)}
                    />
                  )}
                </Col>
              </Row>
                  ...
                  <CheckBox
                    ...
                    onChange={checked => {
                      if (!this.state.checkDeliver) {
                        ...
                        this.setState({
                          checkDeliver: !this.state.checkDeliver,
                          checkToGo: !checked
                        });
                      }
                    }}
                  ...
                  <CheckBox
                    ...
                    onChange={checked => {
                      if (!this.state.checkToGo) {
                        ...
                        this.setState({
                          checkToGo: !this.state.checkToGo,
                          checkDeliver: !checked
                        });
                      }
                    }}
                  />
