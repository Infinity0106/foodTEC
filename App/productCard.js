...
export default class ProductCard extends Component {
  ...
  componentWillMount(){
    var self=this;
    firebase.storage().ref().child("products").child(this.props.data.key+".jpg").getDownloadURL().then((url)=>{
      if(url){
  ...
  renderLeftCol(){
    switch(this.state.loading){
      case true:
        ...
        break;
      case false:
        return(<Image source={{uri:this.state.photoURL}} style={styles.imgStyle}/>)
        break;
      case 'error':
        return(<Animation
          ref={animation => { this.animation2 = animation; if(this.animation2!=null){this.animation2.play()} }}
          style={styles.lottieImg}
  ...
  renderImage(){
    if(this.props.data.type==="in-place"){
      return(<Location width={50} height={50}></Location>)
    }else{
      return(<Deliver width={40} height={40}></Deliver>)
    }
  }
  renderMiddle(){
    return(
      ...
      <Row>
        <Grid>
          ...
          <Col size={70}>
            ...
            <Row>
              <Text style={styles.rightText}>{this.props.data.benefit?"in-benefit of":''}</Text>
            </Row>
          </Col>
        </Grid>
      </Row>
  ...
  render() {
    return (
      <View style={{paddingBottom:10}}>
        ...
        <Animated.View style={{height:this.state.animVal}} collapsed={this.state.collapsable}>
          <AnimTouchableOpacity style={{height:this.state.animVal}} onPress={()=>{Linking.openURL('whatsapp://app')}}>
          <Grid style={[styles.collapsable,{paddingBottom:(this.state.animVal._value>0?3:null)}]}>
            <Row>
              <Text style={styles.tW}>Location: {this.props.data.location}</Text>
            </Row>
            <Row>
              <Text style={styles.tW}>Wapp: {(this.props.data.wapp!=''&&this.props.data.wapp!=undefined?this.props.data.wapp:"no phone")}</Text>
            </Row>
            <Row>
              <Col>
                <Animated.Text style={[styles.tW,{height:this.state.animVal.interpolate({inputRange: [0, 55],outputRange: [0, 15],})}]}>From: 8:00 PM</Animated.Text>
              </Col>
              <Col>
                <Animated.Text style={[styles.tW,{height:this.state.animVal.interpolate({inputRange: [0, 55],outputRange: [0, 15],})}]}>To: 10:00 PM</Animated.Text>
              </Col>
            </Row>
          </Grid>
